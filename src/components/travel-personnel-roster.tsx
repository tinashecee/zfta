...
import {
  formatPersonnelCellValue,
  visiblePersonnelViewColumns,
} from "~/lib/personnel-dossier-display";
import {
  downloadIncomingDelegationTemplateXlsx,
  downloadPersonnelTemplateXlsx,
  parseIncomingDelegationExcelFile,
  parsePersonnelExcelFile,
} from "~/lib/personnel-excel";
import {
  newPersonnelRow,
  PERSONNEL_GENDERS,
  PERSONNEL_STATUSES,
  type PersonnelRosterVariant,
...
export const TravelPersonnelRoster = component$<TravelPersonnelRosterProps>(
  ({ personnel, mode, variant = "full" }) => {
    ...
    const onDownloadTemplate$ = $(async () => {
      if (isDelegation) {
        await downloadIncomingDelegationTemplateXlsx();
      } else {
    ...
    const onFileChange$ = $(async (e: Event) => {
      ...
      if (isDelegation) {
        const result = await parseIncomingDelegationExcelFile(file);
        if (!result.ok) {
    ...
    const viewColumns = isView ? visiblePersonnelViewColumns(personnel) : [];
    ...
    return (
      <div class="space-y-6">
              ...
              {isDelegation ? (
                <>
                  ...
                  <code class="rounded bg-surface-container-high px-1 py-0.5 text-[10px]">
                    full_name, passport_number, country_of_origin, passport_expiry
                  </code>
                  . Dates use <code class="text-[10px]">YYYY-MM-DD</code>. Remove the sample row before upload.
                </>
              ) : (
                <>
                  Template columns:{" "}
                  <code class="rounded bg-surface-container-high px-1 py-0.5 text-[10px]">
                    full_name, gender, date_of_birth, national_id_number, passport_number, passport_expiry, role,
                    position, status
                  </code>
                  . Dates use <code class="text-[10px]">YYYY-MM-DD</code>. Gender:{" "}
                  <code class="text-[10px]">male</code> or <code class="text-[10px]">female</code>. Role is any label
                  you use (use <code class="text-[10px]">player</code> for squad members so player counts stay
                  correct). Remove the sample row before upload.
                </>
              )}
        ...
        {uploadError.value ? (
          <div
            class="rounded-xl border border-error/30 bg-error/5 p-4 text-sm text-error whitespace-pre-wrap"
            role="alert"
          >
            {uploadError.value}
          </div>
        ...
        {personnel.length === 0 ? (
          <div class="rounded-xl border border-dashed border-outline-variant/40 bg-surface-container-low/50 p-8 text-center text-sm text-on-surface-variant">
            {isView
              ? "No travellers on this application."
              : "No travellers yet. Upload a spreadsheet or add people manually."}
          </div>
        ) : isView ? (
            ...
            <table class="w-full min-w-[480px] text-left text-sm">
              <thead>
                <tr class="bg-surface-container-high text-[10px] font-black uppercase tracking-wider text-outline">
                  {viewColumns.map((col) => (
                    <th key={col.key} class="px-3 py-3">
                      {col.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody class="divide-y divide-outline-variant/10">
                {personnel.map((row) => (
                  <tr key={row._clientId} class="bg-surface-container-lowest hover:bg-surface-container-low/80">
                    {viewColumns.map((col) => (
                      <td key={col.key} class="px-3 py-2 align-top text-on-surface">
                        {formatPersonnelCellValue(row, col.key, col)}
                      </td>
                    ))}
                  </tr>
                ))}
            ...
            <table class="w-full min-w-[520px] text-left text-sm">
              <thead>
                <tr class="bg-surface-container-high text-[10px] font-black uppercase tracking-wider text-outline">
                  ...
                  <th class="px-3 py-3">Country of origin</th>
                      ...
                      <td class="px-3 py-2 align-top">
                        ...
                        <td class="px-3 py-2 align-top">
                          <input
                            ...
                            onInput$={(e) => {
                              row.country_of_origin = (e.target as HTMLInputElement).value;
                            }}
                          />
                        </td>
                        <td class="px-3 py-2 align-top">
