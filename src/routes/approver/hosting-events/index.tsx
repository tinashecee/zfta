...
export default component$(() => {
  ...
  return (
    <div class="flex flex-1 flex-col min-h-0 min-w-0 bg-background text-on-background">
      <ApproverPortalNav activeItem="hostingEventsManage" title="Hosting events · SRC" />
          ...
          <header class="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-on-surface-variant text-sm">
                Create and publish bidding events for sport bodies. Uses{" "}
                <code class="text-xs font-mono">/api/hosting/events</code>.
              </p>
            </div>
            <a
              class="inline-flex rounded-xl bg-primary px-5 py-3 text-sm font-bold text-on-primary shadow-lg"
              href="/approver/hosting-events/new/"
            >
              Create event
            </a>
          </header>
          ...
          <div class="overflow-x-auto rounded-xl border border-outline-variant/20 bg-surface-container-lowest shadow-sm">
            <table class="w-full min-w-[56rem] border-collapse text-left text-sm">
              ...
              <tbody class="divide-y divide-outline-variant/10">
                {loading.value ? (
                  <tr>
                    <td class="px-4 py-10 text-center text-outline" colSpan={6}>
                      Loading…
                    </td>
                  </tr>
                ) : loadError.value ? (
                  <tr>
                    <td class="px-4 py-10 text-center text-error" colSpan={6}>
                      {loadError.value}
                    </td>
                  </tr>
                ) : events.length === 0 ? (
                  <tr>
                    <td class="px-4 py-10 text-center text-on-surface-variant" colSpan={6}>
                      No events yet.
                    </td>
                  </tr>
                ) : (
                        ...
                        <td class="px-4 py-3">
                          ...
                          <div class="mt-1 text-xs text-on-surface-variant">
                            <a
                              ...
                              href={`/approver/submitted-bids/?event_id=${encodeURIComponent(ev.id)}`}
                            >
                              View submitted bids
                            </a>
                          </div>
